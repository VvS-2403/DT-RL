...
def train_with_wandb(args):
    ...
    # ── 2. Load Real Data (Dow 30) ──────────────────────────────────────
    import pandas as pd
    import os
    
    csv_path = "dow30_market_data_2013_2022.csv"
    if not os.path.exists(csv_path):
        raise FileNotFoundError(f"Could not find {csv_path}. Please run download_dow30.py first.")
        
    print(f"[1/6] Loading Dow 30 market data from {csv_path}...")
    ...
    args.num_assets = raw_df['Ticker'].nunique()
